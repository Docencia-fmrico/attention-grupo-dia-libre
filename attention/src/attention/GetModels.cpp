...
CallbackReturnT 
GetModels::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  sub_ = create_subscription<gazebo_msgs::msg::ModelStates>(
    "/scan_filtered", 10, std::bind(&model_state_cb, this, _1));
...
CallbackReturnT 
GetModels::on_cleanup(const rclcpp_lifecycle::State & state) 
{
...
CallbackReturnT 
GetModels::on_shutdown(const rclcpp_lifecycle::State & state) 
{
...
CallbackReturnT 
GetModels::on_error(const rclcpp_lifecycle::State & state) 
{
  RCLCPP_INFO(get_logger(), "[%s] Shutting Down from [%s] state...", get_name(), state.label().c_str());
  return CallbackReturnT::SUCCESS;
}
...
void 
model_state_cb(const gazebo_msgs::msg::ModelStates::SharedPtr msg)
{
  bool not_there = true;
